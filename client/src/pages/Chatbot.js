...
export default function () {
  ...
  const diagnoseSymptoms = async (diagnosis, lastAnswer) => {
    ...
    try {
      ...
      if (lastAnswer) setMsgArr([...msgArr, lastAnswer, newMsg]);
      else setMsgArr([...msgArr, newMsg]);
  ...
  useEffect(() => {
    ...
    console.log(msgArr);
    if (msgArr.length < 2) theBot.fun.sendFirst(getBotProps());
  }, [msgArr]);
  ...
  const showIcon = (arr, idx, msg) => {
    if (idx < arr.length - 1) {
      if (arr[idx + 1] && arr[idx + 1].isUser !== msg.isUser) return true;
    } else if (idx === arr.length - 1) return true;
    else return false;
  };
    ...
    switch (prop) {
      ...
      case 'yesnodk':
        if (val === 'done') {
          ...
          for (let i in answers) {
            if (answers[i].includes(textVal)) hasVal = i;
          }
          if (hasVal) handleClick('yesnodk', hasVal, textVal);
        ...
        break;
      case 'agegender':
        if (values.age && values.sex) answerFun.submit(getBotProps());
        break;
      case 'diagnosis':
        if (val === 'close') {
      ...
      case 'text':
        if (val === 'send') {
          ...
          if (hasVal) handleClick('text', hasVal);
          else answerFun.send(getBotProps(), textVal);
          setTextVal('');
        } else if (val === 'done') {
          answerFun.done(getBotProps());
        }
  ...
  const onEnterPress = (prop) => (e) => {
    if (e.keyCode == 13 && e.shiftKey == false) {
      e.preventDefault();
      if (!loading && textVal.length > 0) handleClick(prop, 'send');
    }
  };
  ...
  const handleDelete = (prop, id) => {
    setValues({
      ...values,
      symptoms: values.symptoms.filter((symptom) => {
                        ...
                        <div
                          ...
                          {showIcon(msgArr, idx, msg) && (
                            <div
                              ...
                              {userData && userData.userData && msg.isUser ? (
                                <p>
                          ...
                          <div
                            className={`box ${msg.isUser ? 'user' : 'cekon'}`}>
                            {msg.isFinal ? (
                              <div className="text">
                                {msg.text}
                                <Button
                                  onClick={() => {
                                    handleClick('diagnosis', 'results');
                                  }}
            ...
            <div className="dummy" ref={dummyMsgRef}>
              {values.symptoms.length > 0 && (
                <>
                  ...
                  <div className="symptoms">
                    {values.symptoms &&
                      values.symptoms.map((symptom, idx) => {
                        if (
                          symptom.choice_id === 'present' ||
                          !symptom.choice_id
                        ) {
                          if (!symptom.disabled)
                            return (
                              <Chip
                                ...
                                label={symptom.name}
                                onDelete={() => {
                                  handleDelete('symptoms', symptom.id);
                                }}
                                color="primary"
                                className="chip"
                              />
        ...
        <div className="footer">
          {answerType === 'yesno' ? (
              ...
              <div className="text">
                ...
                <Button
                  ...
                  <img
                    className="icon"
                    src={
                      textVal.length < 1 || loading || disabled
                        ? require('../assets/arrowDisabled.svg')
            ...
            <div className="body agegender">
              ...
              <Button
                onClick={() => {
                  handleClick('agegender');
                }}
                className="btn submit"
                variant="contained"
                color="primary"
                disabled={!values.sex || !values.age}>
                Submit
              </Button>
            </div>
          ) : (
