...
export default function ({ showUser, setShowUser, }) {
    ...
    return (
        <motion.div initial="init" animate="in" exit="out" variants={pageTransition} className='user-outer-container' onClick={() => { setShowUser(null) }}>
            ...
            {data && data.userData &&
                <div className="user-container" onClick={(e) => { e.stopPropagation() }}>
                    ...
                    <div className="btn-panel">
                        <Button onClick={() => { handleClick('history') }} disabled={true} value={values.login} className="btn" variant="contained" color="primary">
                            History
                    </Button>
                        <Button onClick={() => { handleClick('logout') }} value={values.login} className="btn" variant="outlined" color="primary">
                            Logout
                    </Button>
                    </div>
                    <img className="closebtn" alt="Close" src={require('../assets/xmarks.svg')} onClick={() => { setShowUser(null) }} />
